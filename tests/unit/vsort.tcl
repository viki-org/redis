start_server {tags {"vsort"}} {
  proc setup_data {} {
    r del cap anti l:-2l:us incl excl
  ...
  proc basics {encoding} {
    ...
    test "wrong number of parameters - $encoding" {
      ...
      assert_error $err {r vsort cap anti 5}
      assert_error $err {r vsort cap anti 1 a a}
    }

    test "wrong type for count - $encoding" {
      set err "ERR value is not an integer or out of range"
      assert_error $err {r vsort cap anti abc l:-2l:us 0 0 100v}
    }
    ...
    test "returns less results than requested if we don't have enough matches - $encoding" {
      setup_data
      assert_equal {r100 r200} [r vsort cap anti 5 l:-2l:us 0 0 100c 200c]
    }

    test "returns results when the count matches the exact number requested - $encoding" {
      setup_data
      assert_equal {r100 r200 r300} [r vsort cap anti 3 l:-2l:us 0 0 100c 200c 300v]
    }

    test "returns the results limited by most viewed - $encoding" {
      setup_data
      assert_equal {r600 r200 r300} [r vsort cap anti 3 l:-2l:us 0 0 100c 200c 300v 400c 500v 600v]
    }

    test "returns the results limited by most viewed with unranked members - $encoding" {
      setup_data
      assert_equal {r600 r200 r300} [r vsort cap anti 3 l:-2l:us 0 0 1000c 100c 200c 300v 400c 500v 600v]
    }

    test "applies holdbacks - $encoding" {
      setup_data
      r sadd cap 200c 300v
      assert_equal {r600 r100 r500} [r vsort cap anti 3 l:-2l:us 0 0 100c 200c 300v 400c 500v 600v]
    }
