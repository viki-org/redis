...
void vfindCommand(redisClient *c) {
  ...
  if ((getLongFromObjectOrReply(c, c->argv[7], &allow_count, NULL) != REDIS_OK)) { return; }
  ...
  data->allows = loadSetArray(c, 8, &allow_count);
