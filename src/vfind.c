...
#define DIRECTION_ARG_INDEX 5
#define INCLUDE_BLOCKED_ARG_INDEX 6
#define ALLOW_COUNT_ARG_INDEX 7
#define BLOCK_COUNT_ARG_INDEX 8
...
int *vfindGetKeys(struct redisCommand *cmd, robj **argv, int argc, int *numkeys)
{
    ...
    int allow_count = atoi(argv[ALLOW_COUNT_ARG_INDEX]->ptr);
    if (argc < (MIN_ARG_COUNT + allow_count))
    {
        return NULL;
    }

    // count starting offset for block keys
    int block_offset = BLOCK_COUNT_ARG_INDEX + allow_count;
    int block_count = atoi(argv[block_offset]->ptr);

    int filter_offset = FILTER_COUNT_ARG_INDEX + allow_count + block_count;
    if (argc < (MIN_ARG_COUNT + allow_count + block_count))
    {
        return NULL;
    }
    int filter_count = atoi(argv[filter_offset]->ptr);

    int total_keys = 1 + block_count + allow_count + filter_count; // 1 is for the zset
    ...
    {
        keys[offset++] = ALLOW_COUNT_ARG_INDEX + 1 + i; // +1 as start of [allow keys] is after allow_count
    }
...
void vfindCommand(client *c)
{
    ...
    if (!strcasecmp(direction->ptr, "asc"))
    {
        data->desc = 0;
    }
    // The keyword for blocked items is "withblocked"
    if (!strcasecmp(include_blocked->ptr, "withblocked"))
    {
