...
local quicksort_module = require("quicksort")

-- helper functions
local function is_in_all_filters(start_index, item)
...
local function is_blocked(item)
    for i = start_index, block_count do
...
local function is_allowed(item)
    for i = start_index, allow_count do
...
local function vfindBySmallestFilter():
    results = {}
    found = 0
    added = 0

    local smallest_filter = filter_keys[1]
    local filter_set = redis.call("ZRANGE", zset_key, offset, -1)
    if direction == "desc" then
        filter_set = redis.call("ZREVRANGE", zset_key, offset, -1)
    end

    for i = 1, #filter_set do
        local item = filter_set[i]
        ...
        if filter_count > 0 and not is_in_all_filters(2, item) then
            goto continue
        end

        local is_item_blocked = is_blocked(item)
        ...
        if found > offset and added < count then
            local result = {}
            result["id"] = item
            result["blocked"] = is_item_blocked
...
local function vfindByZSet()
    ...
    for i = 1, #zset do
        ...
        if filter_count > 0 and not is_in_all_filters(1, item) then
            goto continue
        end
...
quicksort_module.quicksort(filter_keys, compare_sets_by_cardinality)
size = redis.call("SCARD", filter_keys[1])
ratio = redis.call("SCARD", zset_key) / size

if (size < 100 and ratio > 1) or (size < 500 and ratio > 2) or (size < 2000 and ratio > 3) then
    return vfindBySmallestFilter()
else
    return vfindByZSet()
end